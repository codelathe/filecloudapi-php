...
/**
 * Class AddMetadataSetTest
 * @package CodeLathe\FileCloudApi\Tests\CloudApi
 */
class AddMetadataSetTest extends TestCase
{
    /**
...
<commands>
    <command>
        <type>addmetadataset</type>
        <result>1</result>
        <message>Metadata set definition Review added successfully</message>
    </command>
</commands>
RESPONSE;
        ...
        $cloudApiMock->method('doPost')
            ->with("{$serverUrl}/admin/addmetadataset", http_build_query($mockApiRequest))
            ->willReturn($mockApiResponse);

        /** @var CloudAdminAPI $cloudApiMock */
        /** @var CommandRecord $commandRecord */
        $commandRecord = $cloudApiMock->addMetadataSet(...array_values($this->getValidArguments()));
        $this->assertEquals(1, $commandRecord->getResult());
        $this->assertEquals('addmetadataset', $commandRecord->getType());
        $this->assertEquals('Metadata set definition Review added successfully', $commandRecord->getMessage());
    }
...
<commands>
    <command>
        <type>addmetadataset</type>
        <result>0</result>
        <message>Failed to add new Metadata Set. Reason: Metadata set definition is invalid. name: Set Definition Name has to be specified</message>
    </command>
</commands>
RESPONSE;
        $mockApiRequest = $this->getValidApiRequest();
        ...
        /** @var CloudAdminAPI $cloudApiMock */
        /** @var CommandRecord $commandRecord */
        $commandRecord = $cloudApiMock->addMetadataSet(...array_values($arguments));
        $this->assertEquals(0, $commandRecord->getResult());
        $this->assertEquals('addmetadataset', $commandRecord->getType());
        $this->assertEquals(
            'Failed to add new Metadata Set. Reason: Metadata set definition is invalid. name: Set Definition Name has to be specified',
